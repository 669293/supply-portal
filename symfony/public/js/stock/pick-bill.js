$(document).ready(function() {
  ...
  function sendForm() {
      ...
      $('.application-input').attr('name', '');
      $('.bill-input').attr('name', '');
    
      $('.material-select:checked').each(function() {
        $(this).closest('tr').find('.amount-input').attr('name', 'amount[]');
        $(this).closest('tr').find('.application-input').attr('name', 'application[]');
        $(this).closest('tr').find('.bill-input').attr('name', 'bill[]');
      });
      ...
      $.ajax({
          ...
          if (data[0] == 1) {
            location.href = 'http://vitim.1gb.ru/model/php/setPMLogistic.php?type=pm&secret=35b7fff08b557bd2909fc91316fc6216&doc=' + docID + '&logistic=' + JSON.stringify(data[1]) + '&appinfo=' + encodeURI(data[2]);
          } else {
