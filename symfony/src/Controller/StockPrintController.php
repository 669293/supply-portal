...
HERE;
        ...
        foreach ($objMaterials as $material) {
            ...
            echo "            <td class=\"bordered\">".number_format($material->sum, 2, ".", " ")."</td>\n";
            echo "            <td class=\"bordered\">".number_format($material->tax, 2, ".", " ")."</td>\n";
            echo "            <td class=\"bordered\" style=\"border-right-width: 2px;\">".number_format($material->total, 2, ".", " ")."</td>\n";
            echo "            <td class=\"bordered\"></td>\n";
            echo "            <td class=\"bordered\"></td>\n";
            echo "        </tr>\n";
...
HERE;
        ...
        foreach ($objMaterials as $material) {
            ...
            echo "            <td class=\"bordered\">".number_format($material->obj->getPrice(), 2, ".", ",")."</td>\n";
            echo "            <td class=\"bordered\">".number_format($material->total, 2, ".", ",")."</td>\n";
...
HERE;
        ...
        $mpdf->keep_table_proportions = TRUE;
        $mpdf->shrink_tables_to_fit=1;
        $mpdf->SetTitle('Просмотр требования накладной'.$objStock->getId());
        $mpdf->WriteHTML($css, \Mpdf\HTMLParserMode::HEADER_CSS);
        $mpdf->WriteHTML($content, \Mpdf\HTMLParserMode::HTML_BODY);
        
        $mpdf->Output('Требование накладная '.$objStock->getId().'.pdf', 'I');
    }
...
HERE;        
        ...
        $mpdf->keep_table_proportions = TRUE;
        $mpdf->shrink_tables_to_fit=1;
        $mpdf->SetTitle('Просмотр списания материалов'.$objStock->getId());
        $mpdf->WriteHTML($css, \Mpdf\HTMLParserMode::HEADER_CSS);
        $mpdf->WriteHTML($content, \Mpdf\HTMLParserMode::HTML_BODY);
        
        $mpdf->Output('Списание материалов '.$objStock->getId().'.pdf', 'I');
    }

    /**
     * @Route("/stock/print/lzk", methods={"GET"})
     * @Security("is_granted('ROLE_STOCK') or is_granted('ROLE_BUH')")
     */
    public function printLZK(
        Request $request,
        ProvidersRepository $providersRepository,
        StockRepository $stockRepository,
        StockMaterialsRepository $stockMaterialsRepository,
    ...
    ): Response
    {
        ...
        echo '<div class="block" style="text-align: right;">Типовая межотраслевая форма № М-11<br />Утверждена постановление Госкомстата России от 30.10.97 № 71а</div>'."\n";
        echo "<div class=\"block\" style=\"text-align: center;\"><h1>ЛИМИТНО-ЗАБОРНАЯ КАРТА № ".$num."</h1></div>\n";
        echo <<<HERE
        <div class="block">
            <table>
                <tr>
                ...
                <tr>
                    <td style="text-align: left;">Структурное подразделение:</td>
HERE;
        echo "            <td colspan=\"3\" style=\"border-bottom: 1px solid #000; text-align: left;\">База Иркутск</td>\n";        
        echo <<<HERE
                </tr>
            </table>
        </div>
        <div class="block" style="margin-top: 20px;">
            <table>
                <tr>
                    <td class="bordered">Дата составления</td>
                    <td class="bordered">Код вида операции</td>
                    <td class="bordered">Списано на автомобиль</td>
                </tr>
                <tr style="border: 2px solid #000;">
HERE;
        ...
        echo "            <td class=\"bordered\">".$objStock->getDate()->format('d.m.Y')."</td>\n";
        echo '            <td class="bordered"></td>'."\n";
        echo "            <td class=\"bordered\">".$objStock->getTransport()->getTitle()."</td>\n";
        echo <<<HERE
                </tr>
            </table>
        </div>
...
HERE;
            ...
            echo <<<HERE
                <tr>                                                                                                                                                                                                                             
                    <td colspan="6" style="font-weight: bold; padding-top: 10px; text-align: right;">Всего по лимитно-заборной карте на сумму, руб:</td>
HERE;
        ...
        <div class="block">
HERE;
            echo '<img src="'.getcwd().'/img/signatures/signature_0.png" alt="" style="height: 75px; position: absolute; margin-left: 135px; margin-bottom: -80px; width: 180px;" />'."\n";
        
            if (!empty($objStock->getTransport()->getSignature())) {
                echo '    <img src="'.getcwd().'/img/signatures/'.$objStock->getTransport()->getSignature().'" alt="" style="'.$objStock->getTransport()->getStyle().'" />'."\n";
            }
            ...
            <table>
                <tr>
                    ...
                    <td></td>
                    <td style="width: 60px; text-align: right;">Получил</td>
                    <td style="border-bottom: 1px solid #000; width: 13%;">водитель</td>
                    <td style="width: 5px;"></td>
                    <td style="border-bottom: 1px solid #000; width: 13%;"></td>
                    <td style="width: 5px;"></td>
HERE;
        
            echo "            <td style=\"border-bottom: 1px solid #000; width: 13%;\">".$objStock->getTransport()->getDriver()."</td>\n";
